  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex flex-col justify-between items-start">
        ...
        <h2 className="m-10 text-2xl font-bold mb-5 border-gray-300 border-b-1 pb-7 w-[80%]">Shopping Cart</h2>
        <div className="p-10 w-full h-[250px] flex flex-col overflow-y-auto">
          {cartItems.length === 0 ? (
            <p className="text-gray-600 m-20">Your cart is empty.</p>
          ) : (
        ...
        <div className="mt-10 py-3 px-10 w-full flex items-center justify-between gap-10">
          <p className="text-xl text-gray-700 font-light">Subtotal</p>
          <h6 className="text-2xl text-[#6b5a22] font-bold">Rs. {calculateSubtotal().toLocaleString()}</h6>
        </div>
        <nav className="w-full py-5 px-10 flex justify-between items-center md:gap-10 border-t-1 border-gray-300">
            <Link
              href="/cart"
              onClick={onRequestClose}
              className="relative py-3 px-7 md:px-10 text-xl rounded-4xl bg-white border-1 border-gray-700 text-black hover:text-white hover:bg-gray-900 transition-colors duration-300"
            >
              View Cart
            </Link>
            <Link
